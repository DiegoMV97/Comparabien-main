...
namespace ConcentradorBackend.DataAccess
{
    public class ConsultaProductoDataAccessLayer: IConsultaProductoService
    {
        ...
        public ConsultaEntidadProducto DetalleProducto(int CodigoProductoFinanciero)
        {
            try
            {
                var consulta = from cep in _dbContext.ConsultaEntidadProducto
                               select cep;
                consulta = consulta.Where(s => s.ConsultaEntidadProductoId == CodigoProductoFinanciero);
                return (ConsultaEntidadProducto)consulta.AsNoTracking().SingleOrDefault();
            }
            catch (Exception e)
            {
                throw e;
